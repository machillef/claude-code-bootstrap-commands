...
If `docs/ai/<initiative>-status.md` does not exist, stop: "No initiative found for `<initiative>`. Run `/bootstrap-existing <initiative>` or `/bootstrap-new <initiative>` first."

If any slice is in "Needs Fix" state, stop: "Slice <N> has reported bugs. Run `/fix-bugs <initiative>` first, then resume with `/loop-work <initiative>`."
...
- **Step 6:** Bounded plugin delegation (ECC agents if available)
...
4. **Slice in Needs Fix state** → STOP. Tell user to run `/fix-bugs`.
...
Next:
  ...
  - /fix-bugs <initiative> — if QA reveals bugs
...
2. **Do NOT ask "shall I continue?"** between slices. The user authorized the full loop by invoking `/loop-work`.
