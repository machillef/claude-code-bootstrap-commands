...
**Triage gate:** If the new objective is materially different from the original design (new domain, different architecture, unrelated feature), recommend `/initiative` for a fresh initiative instead of extending. Extend mode is for follow-up work within the same design, not for starting something new.
