...
The skill handles: confirm greenfield → requirements → design exploration → stack decision (wait for user confirmation) → scaffold → docs/ai/ → ECC wiring → stop.

If the user already has code, redirect to `/bootstrap-existing` instead.
